...
async function createTestPlan() {
  ...
  if (!json.success || !json.data?.id) {
    throw new Error(`Create plan failed: ${JSON.stringify(json)}`);
  }
...
async function testGraphApi() {
  ...
  const { planId, nodeId } = await createTestPlan();
  ...
  try {
    ...
    if (json.success && json.data.nodes && json.data.edges) {
      ...
      console.log(
        `   Received ${json.data.nodes.length} nodes, ${json.data.edges.length} edges`,
      );
    } else {
  ...
  try {
    const res = await fetch(
      `${BASE_URL}/plans/${planId}/nodes/${nodeId}/status`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
    ...
    if (json.success && json.data.status === "learned") {
      ...
      console.log(
        `   Progress updated: ${json.data.plan.progress}/${json.data.plan.total}`,
      );
    } else {
  ...
  try {
    const res = await fetch(
      `${BASE_URL}/plans/${planId}/nodes/${nodeId}/position`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
