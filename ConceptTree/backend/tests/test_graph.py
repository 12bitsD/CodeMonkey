...
def client():
    ...
    # Seed data
    user_id = "u_test"
    conn.execute("INSERT INTO users (id, email, password_hash) VALUES (?, ?, ?)", (user_id, "test@example.com", "pw"))
    ...
    plan_id = "p_1"
    conn.execute("INSERT INTO plans (id, user_id, title) VALUES (?, ?, ?)", (plan_id, user_id, "Test Plan"))
...
def test_update_node_status_persists_and_updates_profile(client):
    ...
    session = conn.execute("SELECT * FROM learning_sessions WHERE node_id = 'n_1'").fetchone()
    ...
    profile = conn.execute("SELECT mastered_knowledge FROM user_profiles WHERE user_id = 'u_test'").fetchone()
...
def test_update_node_position_persists(client):
    response = client.put("/api/plans/p_1/nodes/n_1/position", json={"x": 100, "y": 200})
    assert response.status_code == 200
