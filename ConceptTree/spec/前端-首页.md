...
一致性标记：❌（AI/生成/创建计划仍为 Mock；首页→图谱存在断链风险）
...
- 当前 `plansApi.create()` 仍为前端 Mock，会生成本地 planId，但并不会在后端创建对应的 plan。
- 当前 GraphPage 进入后会调用后端 `GET /api/plans/{planId}/graph` 拉取图谱数据；若后端不存在该 planId，会导致“未找到学习计划/图谱加载失败”。
...
| 功能         | 说明                                     |
| ------------ | ---------------------------------------- |
| 拆分建议弹窗 | 当 `shouldSplit=true` 时显示，目前未实现 |
| 真实认证     | 调用后端 `/api/auth/login`               |
| 错误提示     | 解析/生成失败时的Toast提示               |
