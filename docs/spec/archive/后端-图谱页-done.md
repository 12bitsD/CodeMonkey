...
- [x] GET `/api/plans/{plan_id}/graph` - 获取图谱（已接入鉴权）✅
...
| ---- | --------------------------------------------- | -------------- | ---------- | -------------- | ---- | --------------------------------------------------------------------------------------------------------------- |
| GET  | `/api/plans/{plan_id}/graph`                   | 获取图谱       | ✅         | ✅             | ✅   | [graph.py](file:///Users/bytedance/Desktop/CodeSpace/CodeMonkey/ConceptTree/backend/routers/graph.py#L29-L84)   |
...
4. 用户确认后，调用 `POST /api/plans/{plan_id}/apply-changes` 执行变更
...
表结构与约束见 [后端-通用规范.md](./后端-通用规范.md) 的“数据库设计汇总”（Supabase/Postgres 版本）。
